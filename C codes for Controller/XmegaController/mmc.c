...
/* Port Controls  (Platform dependent) */
#define CS_LOW()   PORTE.OUTCLR = 0x10    /* MMC CS = L */
#define CS_HIGH()  PORTE.OUTSET = 0x10    /* MMC CS = H */

#define SOCKPORT  PORTH.IN      /* Socket contact port */
#define SOCKWP    (PORTH.IN & 0x01)          /* Write protect switch (PH0) */
#define SOCKINS   (PORTH.IN & 0x02)          /* Card detect switch (PH1) */
...
{
	do {
		SPIE.DATA = *p++; loop_until_bit_is_set(SPIE.STATUS,7);
		SPIE.DATA = *p++; loop_until_bit_is_set(SPIE.STATUS,7);
	} while (cnt -= 2);
}
